...
d3.json(
	...
	L.geoJson(data, {
		...
		onEachFeature: function (feature, layer) {
			...
			layer.bindPopup(
				`<h3> Magnitude: ${mag} </h3> <hr><h3> Location: ${place}</h3>`
			);
		},
	...
	d3.json(
		...
		L.geoJSON(data, {
			style: myStyle,
			onEachFeature: function (feature, layer) {
				const { Name, PlateA, PlateB } = feature.properties;
				layer.bindPopup(
					`<h3> Plate Name: ${Name} </h3> <hr><h3> Plate A: ${PlateA}</h3>  <hr><h3> Plate B: ${PlateB}</h3>`
				);
			},
